...
chown system.system $LOG_PATH/AtraceLogs
...
# add atrace logs catching for systrace debug performance
elif test "$OPT" = "atrace"
then
rm -f $LOG_PATH/AtraceLogs/list_categories.txt $LOG_PATH/AtraceLogs/categories.txt
/system/bin/atrace --list_categories > $LOG_PATH/AtraceLogs/list_categories.txt
while read categories; do echo -n ${categories%-*}" " >> $LOG_PATH/AtraceLogs/categories.txt; done < $LOG_PATH/AtraceLogs/list_categories.txt
categories=`cat $LOG_PATH/AtraceLogs/categories.txt`
/system/bin/atrace -z -t 20 -b 40960 $categories > $LOG_PATH/AtraceLogs/atrace_$DATE_INFO.log 
fi
