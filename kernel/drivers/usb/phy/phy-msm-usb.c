	...
	enum msm_otg_phy_reg_mode mode)
{
	...
	case USB_PHY_REG_ON:
		...
		if (ret < 0) {
			pr_err("%s: Unable to set HPM of the regulator "
				"HSUSB_1p8\n", __func__);
			printk("%s: Unable to set HPM of the regulator "
				"HSUSB_1p8\n", __func__);
			return ret;
		}
		...
		if (ret) {
			dev_err(motg->phy.dev, "%s: unable to enable the hsusb 1p8\n",
				__func__);
			printk("%s: unable to enable the hsusb 1p8\n",
				__func__);
		...
		if (ret) {
			dev_err(motg->phy.dev, "%s: unable to enable the hsusb 3p3\n",
				__func__);
			printk( "%s: unable to enable the hsusb 3p3\n",
				__func__);
	...
	case USB_PHY_REG_OFF:
		...
		if (ret) {
			dev_err(motg->phy.dev, "%s: unable to disable the hsusb 1p8\n",
				__func__);
			printk("%s: unable to disable the hsusb 1p8\n",
				__func__);
			return ret;
		}
		...
		if (ret < 0){
			pr_err("%s: Unable to set LPM of the regulator "
				"HSUSB_1p8\n", __func__);
			printk("%s: Unable to set LPM of the regulator "
				"HSUSB_1p8\n", __func__);
		}
		...
		if (ret) {
			dev_err(motg->phy.dev, "%s: unable to disable the hsusb 3p3\n",
				 __func__);
			printk("%s: unable to disable the hsusb 3p3\n",
				 __func__);
			return ret;
		}
	...
	case USB_PHY_REG_LPM_ON:
		...
		if (ret < 0) {
			pr_err("%s: Unable to set LPM of the regulator: HSUSB_1p8\n",
				__func__);
			printk("%s: Unable to set LPM of the regulator: HSUSB_1p8\n",
				__func__);
			return ret;
		}
		...
		if (ret < 0) {
			pr_err("%s: Unable to set LPM of the regulator: HSUSB_3p3\n",
				__func__);
			printk("%s: Unable to set LPM of the regulator: HSUSB_3p3\n",
				__func__);
	...
	case USB_PHY_REG_LPM_OFF:
		...
		if (ret < 0) {
			pr_err("%s: Unable to set HPM of the regulator: HSUSB_1p8\n",
				__func__);
			printk("%s: Unable to set HPM of the regulator: HSUSB_1p8\n",
				__func__);
			return ret;
		}
		...
		if (ret < 0) {
			pr_err("%s: Unable to set HPM of the regulator: HSUSB_3p3\n",
				__func__);
			printk("%s: Unable to set HPM of the regulator: HSUSB_3p3\n",
				__func__);
			regulator_set_optimum_mode(hsusb_1p8, USB_PHY_REG_ON);
			return ret;
		}
	...
	default:
		...
		printk("%s: Unsupported mode (%d).", __func__, mode);
		return -ENOTSUPP;
	}
...
#ifdef TINNO_CHG_DET
extern int check_usb_chg_pluhhed_in(void);
int time_s_probe=0;
int time_s=0;
#endif
				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		...
		if(otg_vbus_state==0) {
#endif			
		printk("otg_power_get_property_usb : current_max=%d time_s=%d \n", motg->current_max,time_s);	
		if((motg->chg_type==0)&&((time_s-time_s_probe)>=3)){
				if(check_usb_chg_pluhhed_in())
				{
					if(motg->chg_type==0)
					{
						printk("chg type is null,but vbus has voltage,we set the chg type and current self!\n");
						motg->chg_type=USB_SDP_CHARGER;
						msm_otg_notify_charger(motg,IDEV_ACA_CHG_LIMIT);
						/*if(motg->current_max==0)
							{
							printk("set current_max =500\n");
							motg->current_max=IDEV_ACA_CHG_LIMIT*1000;
						}*/
					}
				}
			}else if(((time_s-time_s_probe)>=3)&&((time_s-time_s_probe)<4))
			{

						printk("chg type is not null,vbus has voltage,we set the chg type and current self!chg_type:%d \n",motg->chg_type);
					//	motg->chg_type=USB_SDP_CHARGER;
					//	msm_otg_notify_charger(motg,IDEV_ACA_CHG_LIMIT);
					if(motg->current_max<=200)
					{
						if(check_usb_chg_pluhhed_in())
						{
							motg->chg_type=USB_SDP_CHARGER;
