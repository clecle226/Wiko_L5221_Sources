...
static enum hrtimer_restart bma2x2_timer_func(struct hrtimer *timer)
{
	...
	bma2x2 = container_of(timer, struct bma2x2_data, poll_timer);

	queue_work(bma2x2->data_wq, &bma2x2->work.work);
	hrtimer_forward_now(&bma2x2->poll_timer,
			ns_to_ktime(atomic_read(&bma2x2->delay)*1000000));
...
static ssize_t bma2x2_remain_times_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{

	int err, remain_times;
	unsigned char databuf;
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);

	err = bma2x2_smbus_read_byte(bma2x2->bma2x2_client, BMA2X2_EEPROM_CTRL_REG,
					&databuf);
	if (err < 0) {
		dev_err(&client->dev, "read eeprom control reg error\n");
		return err;
	}

	printk("%s databuf=%x\n", __func__, databuf);
	
	remain_times = (databuf>>4)&0x0F;
...
static DEVICE_ATTR(remain_times, S_IRUSR|S_IRGRP,
		bma2x2_remain_times_show, NULL);
#ifdef CONFIG_SIG_MOTION
		...
		const struct i2c_device_id *id)
{
	...
	data->use_hrtimer = pdata->use_hrtimer;
